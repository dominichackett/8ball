...
// --- Bot Configuration ---
const POLLING_INTERVAL = 300000//1800000; // 30 minutes for live price checks
...
const historicalPrices = new Map<string, { timestamp: number; price: number; }[]>(); // coinId -> [{timeswhat tamp, price}]
const lastConfidenceCheck = new Map<string, number>(); // coinId -> timestamp
...
// --- Main Application ---
async function main() {
  console.log('Starting Intraday Swing Trading Bot...');
  ...
  console.log('Bot is running. Analyzing daily charts with hourly live price updates.');
}
...
async function initializeTradableTokens() {
   ...
   TRADABLE_TOKENS.set('yieldbricks', { address: '0x11920f139a3121c2836e01551d43f95b3c31159c', chain: 'evm', specificChain: 'evm', symbol: 'YBR' });
    TRADABLE_TOKEN_ADDRESSES.add('0x11920f139a3121c2836e01551d43f95b3c31159c');

    TRADABLE_TOKENS.set('fuel-network', { address: '0x675b68aa4d9c2d3bb3f0397048e62e6b7192079c', chain: 'evm', specificChain: 'evm', symbol: 'FUEL' });
   TRADABLE_TOKEN_ADDRESSES.add('0x675b68aa4d9c2d3bb3f0397048e62e6b7192079c');
  ...
  try {
    ...
    if (agentBalances) {
        ...
        agentBalances.balances.forEach((balance: any) => {
            if ((balance.symbol === 'USDC' || balance.symbol ==='USDbC' ) && balance.chain === 'evm') {
        ...
        const svmUsdcBalance = agentBalances.balances
            .filter((balance: any) => (balance.symbol === 'USDC' || balance.symbol ===  'USDbC') && balance.chain === 'svm')
    ...
    await monitorOpenPositions(portfolio, tradeableEvmUsdcBalances, svmUsdcBalance);
...
interface AnalyzedToken {
    ...
    aiReason?: string; // Added AI reason to interface
}
...
async function analyzeMomentumOpportunities(marketData: any[]): Promise<AnalyzedToken[]> {
    ...
    for (const coin of marketData) {
        ...
        const now = Date.now();
        const lastCheck = lastConfidenceCheck.get(coin.id) || 0;
        const shouldCheckConfidence = isOpportunity || (now - lastCheck > 3600000); // 1 hour in milliseconds

        let aiConfidence: number | undefined = undefined;
        let aiReason: string | undefined = undefined;
        if (shouldCheckConfidence) {
            const analyzedTokenForConfidence: Omit<AnalyzedToken, 'aiConfidence' | 'aiReason'> = {
                ...coin,
                ...tokenInfo,
            ...
            const confidenceResult = await getTradeConfidence(analyzedTokenForConfidence as AnalyzedToken);
            aiConfidence = confidenceResult.confidence;
            aiReason = confidenceResult.reason;
            lastConfidenceCheck.set(coin.id, now);
        }
        ...
        const analyzedToken: AnalyzedToken = {
            ...
            isOpportunity,
            aiConfidence: aiConfidence, // Assign the confidence score
            aiReason: aiReason, // Assign the AI reason
        };
...
async function monitorOpenPositions(portfolio: any, tradeableEvmUsdcBalances: Map<string, { amount: number, address: string }>, svmUsdcBalance: number) {
  console.log("Monitoring open positions...");

  for (const token of tradeManager.getOpenTrades()) {
    if (!TRADABLE_TOKEN_ADDRESSES.has(token.toToken)) { // Corrected to check toToken
      continue;
    }

    const entryPrice = token.price;
    const currentPriceResult = await recall.getPrice({ token: token.toToken, chain: token.toChain, specificChain: token.toSpecificChain });

    if (currentPriceResult && currentPriceResult.price) {
      ...
      if (stopLossHit || takeProfitHit) {
        ...
        const saleProceeds = parseFloat(token.toAmount) * currentPrice;
        console.log(`Executing exit for ${token.toTokenSymbol}: ${reason} (PnL: ${(pnl * 100).toFixed(2)}%). Estimated proceeds: ${saleProceeds.toFixed(2)} USDC`);
        
        const toTokenAddress = token.fromChain === 'svm' ? USDC_SVM_ADDRESS : USDC_ADDRESS;

        try {

          console.log({
            fromToken: token.toToken,
            toToken: toTokenAddress,
            amount: token.toAmount, // Use the toAmount from the original trade
            reason: reason,
            chain: token.toChain,
            specificChain: token.toSpecificChain,
          })
          await recall.executeTrade({
            fromToken: token.toToken,
            toToken: toTokenAddress,
          ...
          // Update internal balance immediately
          if (token.fromChain === 'svm') {
            svmUsdcBalance += saleProceeds;
          } else {
            const evmBalance = tradeableEvmUsdcBalances.get(token.fromSpecificChain);
            if (evmBalance) {
              evmBalance.amount += saleProceeds;
            } else {
              // This case should ideally not happen if the trade was made from a known balance
              tradeableEvmUsdcBalances.set(token.fromSpecificChain, { amount: saleProceeds, address: toTokenAddress });
            }
          }
          console.log(`Updated internal ${token.fromChain.toUpperCase()} balance. New estimated balance: ${token.fromChain === 'svm' ? svmUsdcBalance : tradeableEvmUsdcBalances.get(token.fromSpecificChain)?.amount}`);

          await tradeManager.removeOpenTrade(token.id);

        } catch (error) {
          console.error(`Error executing exit trade for ${token.toTokenSymbol}:`, error);
        }
      }
    } else {
      console.error(`Could not fetch current price for ${token.toTokenSymbol}. Skipping exit check.`);
    }
  }
}
...
async function executeTrades(opportunities: AnalyzedToken[], portfolio: any, tradeableEvmUsdcBalances: Map<string, { amount: number, address: string }>) {
  ...
  const svmUsdcBalance = portfolio.tokens.find((token: any) => token.token === USDC_SVM_ADDRESS && token.chain === 'svm')?.amount || 0;

  if (totalEvmUsdcBalance === 0 && svmUsdcBalance === 0) {
    console.log("No USDC found in portfolio across any chain. Skipping new trades.");
    return;
  }
  ...
  for (const opportunity of opportunities) {
    ...
    const confidence = opportunity.aiConfidence !== undefined ? opportunity.aiConfidence : 0;
    const positionSize = calculatePositionSize(portfolio.totalValue, 0.5); // Consider basing this on total portfolio value

    let fromTokenAddress: string | undefined;
    let fromChain: 'evm' | 'svm' | undefined;
    let fromSpecificChain: string | undefined;

    if (opportunity.chain === 'svm') {
        if (svmUsdcBalance >= positionSize) {
        ...
        for (const chain of preferredEvmChains) {
            ...
            if (evmBalanceInfo && evmBalanceInfo.amount >= positionSize) {
                fromTokenAddress = evmBalanceInfo.address;
                fromChain = 'evm';
                fromSpecificChain = chain;
                break; // Found a suitable chain
            }
        }
    }

    if (!fromTokenAddress || !fromChain || !fromSpecificChain) {
        console.log(`Insufficient USDC balance on any suitable chain for ${opportunity.symbol.toUpperCase()} (requires ${positionSize.toFixed(2)} USDC). Skipping trade.`);
        continue;
    }

    const reason = opportunity.aiReason ? `AI Reason: ${opportunity.aiReason}` : `AI-driven momentum trade. Confidence: ${(confidence * 100).toFixed(0)}%.`;
    console.log(`Executing entry for ${opportunity.symbol.toUpperCase()} from ${fromSpecificChain.toUpperCase()} with size ${positionSize.toFixed(2)} USDC.`);
    ...
    try {
      ...
      // Update internal balance immediately after trade execution
      if (fromChain === 'svm') {
        // This assumes you have a way to update the svmUsdcBalance
        // Since it's passed by value, you might need to return it or handle state differently
      } else {
        const evmBalance = tradeableEvmUsdcBalances.get(fromSpecificChain);
        if (evmBalance) {
          evmBalance.amount -= positionSize;
          console.log(`Deducted ${positionSize} from ${fromSpecificChain.toUpperCase()} balance. New estimated balance: ${evmBalance.amount}`);
        }
      }
...
// --- AI and Performance ---
async function getTradeConfidence(analyzedToken: AnalyzedToken): Promise<{ confidence: number; reason: string }> {
    ...
    try {
        ...
        const reason = aiResponse.replace(/<score>([0-9.]+)<\/score>/, '').trim();
